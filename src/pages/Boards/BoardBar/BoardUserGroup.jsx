...
function BoardUserGroup({ boardUsers = [], limit = 8 }) {
  ...
  // Lưu ý ở đây không dùng Component AvatarGroup của MUI bởi nó không hỗ trợ tốt trong việc chúng ta cần custom & trigger xử lý phần tử tính toán cuối, đơn giản là cứ dùng Box và CSS - Style đám Avatar cho chuẩn kết hợp tính toán một chút thôi.
  return (
    <Box sx={{ display: 'flex', gap: '4px' }}>
      ...
      {[...Array(16)].map((_, index) => {
        if (index < limit) {
          return (
            <Tooltip title="truonglamdev" key={index}>
              <Avatar
